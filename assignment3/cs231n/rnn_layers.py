...
def word_embedding_forward(x, W):
    ...
    # for each value in x (word index) choose row from W
    out = W[x, :]
    cache = (x, W)
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    cache = (x, Wx, Wh, b, i, f, o, g, prev_h, prev_c, tanh_next_c)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    x, Wx, Wh, b, i, f, o, g, prev_h, prev_c, tanh_next_c = cache

    dc = dnext_h * o * (1 - tanh_next_c ** 2) + dnext_c
...
def lstm_backward(dh, cache):
    ...
    for t in reversed(range(T)):
        dprev_h = dh[:, t, :] + dprev_h
        dx_t, dprev_h, dprev_c, dWx_t, dWh_t, db_t = lstm_step_backward(dprev_h, dprev_c, cache[t])
        dx[:, t, :] = dx_t
        dWx += dWx_t
        dWh += dWh_t
        db += db_t
