...
def max_pool_forward_naive(x, pool_param):
    ...
    W_out = int((H - pool_width) / stride + 1)
        ...
        for j in range(C):
            for k in range(H_out):
                for l in range(W_out):
                    ...
                    l0 = l * stride
                    l1 = l0 + pool_width
                    out[i, j, k, l] = np.max(x[i, j, k0:k1, l0:l1])
        ...
        for j in range(C):
            for k in range(H_out):
                for l in range(W_out):
                    ...
                    l0 = l * stride
                    l1 = l0 + pool_width
                    x_cropped = x[i, j, k0:k1, l0:l1]
                    x_max_mask = x_cropped == np.max(x_cropped)
                    dx[i, j, k0:k1, l0:l1] += x_max_mask * dout[i, j, k, l]
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    N, C, H, W = x.shape
    x_bn = np.transpose(x, (0, 2, 3, 1)).reshape((N * H * W, C))
    out_bn, cache = batchnorm_forward(x_bn, gamma, beta, bn_param)
    out = np.transpose(out_bn.reshape((N, H, W, C)), (0, 3, 1, 2))
...
def spatial_batchnorm_backward(dout, cache):
    ...
    N, C, H, W = dout.shape
    dout_bn = np.transpose(dout, (0, 2, 3, 1)).reshape((N * H * W, C))
    dx_bn, dgamma, dbeta = batchnorm_backward(dout_bn, cache)
    dx = np.transpose(dx_bn.reshape((N, H, W, C)), (0, 3, 1, 2))
